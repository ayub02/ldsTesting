...
def write_opc_values(_dict_PT_tag_loc, _dict_PT_tag_station, _dict_PT_tag_value, _flow_up, _flow_down, _prod_temp_MKT,
                     _prod_temp_KBS, _soil_temp_seg1, _norm_density, _reinj_flow, _leak_location, _leak_pdrop):
    # Writing PT values
    ...
    TT_dict = {'TT2003': _prod_temp_MKT, 'TT3011': _prod_temp_KBS}
    ...
    FT_dict = {'FT2080': _flow_up * _norm_density, 'FT3010': _flow_down * _norm_density}
    publislhBuffer('FT2080', FT_dict['FT2080'], _topic='MKT')
    publislhBuffer('FT3010', FT_dict['FT3010'] + _reinj_flow, _topic='KBS')
...
# Reading instruments
PTs = pd.read_excel('../data/instrumentation_POC.xlsx', sheet_name='PT')
...
# Reading topology
chainage = pd.read_excel('../data/MKT_KBS_data_points_400m_new.xlsx', sheet_name=0)
mfmPOC = Pipeline(node1_dist=chainage['dist_node1'].tolist(),
...
if shut_in is False:
    if is_leak is False:
        locations, pressures, flow_up, flow_down = mfmPOC.leak_noleak_pressures_flows(upstream_pressure, noLeakFlow,
        ...
        write_opc_values(dict_PT_tag_loc, dict_PT_tag_station, dict_PT_tag_value, flow_up, flow_down, prod_temp_MKT,
                         prod_temp_KBS, soil_temp_seg1, norm_density, reinj_flow, 0, 0)

    if is_leak is True:
        for index, val in enumerate(leak_pdrop_array):
            print('')
            print('*************************************************')
            print('Step', index+1, '/', len(leak_pdrop_array))
            print('*************************************************\n')
            sleep(2)
            locations, pressures, flow_up, flow_down = mfmPOC.leak_noleak_pressures_flows(upstream_pressure, noLeakFlow,
                                                                                          is_leak=True,
            ...
            write_opc_values(dict_PT_tag_loc, dict_PT_tag_station, dict_PT_tag_value, flow_up, flow_down, prod_temp_MKT,
                             prod_temp_KBS, soil_temp_seg1, norm_density, reinj_flow, leak_location, val)
    ...
    if is_leak is True:
        ...
        PT_tag_value, PT_tag_station = write_opc_values(dict_PT_tag_loc, dict_PT_tag_station, dict_PT_tag_value,
                                                        flow_up, flow_down, prod_temp_MKT, prod_temp_KBS,
                                                        soil_temp_seg1, norm_density, reinj_flow, 0, 0)
